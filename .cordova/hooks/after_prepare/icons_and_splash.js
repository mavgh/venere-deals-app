...
projectConfig.doc.findall('splash').map(function (node) {
  if (/\/$/.test(node.attrib['src']) && node.attrib['gap:platform'] === undefined && node.attrib['gap:density'] === undefined) {

    if (~projectPlatforms.indexOf('android')) {
      // Android
      copyAsset('splash', { attrib: { 'gap:platform': 'android', src: node.attrib.src + 'android/screen-ldpi-portrait.png', 'gap:density': 'ldpi' } });
      copyAsset('splash', { attrib: { 'gap:platform': 'android', src: node.attrib.src + 'android/screen-mdpi-portrait.png', 'gap:density': 'mdpi' } });
